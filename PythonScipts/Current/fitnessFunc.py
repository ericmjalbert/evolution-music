...
#----------------------------------------------------------------------
#   Fitness Function:
#       Opens each external fitness function and then takes a weighted 
#       average of the scores for each member in the population.
...
def arpeggio(song):
    ...
    for i in range(len(song) - 4):
        ...
        if noteDiff1 == 4 and noteDiff2 == 3: fitness += 0.5
        if noteDiff1 == 3 and noteDiff2 == 4: fitness += 0.5
        if noteDiff3 == 7 or noteDiff3 == 6: fitness += 0.5
...
def longBass(song):
    ...
    for note in song:
        ...
        if pitch > 59 and dur < 64: fitness += 1
        if pitch < 51 and dur > 92: fitness += 1 
...
def scaleC(song):
    ...
    for note in song:
        pitch = midiNote.getPitch(note)
        if pitch%12 in [0, 2, 3, 5, 7, 8, 10]: fitness += 1
    return fitness/float(len(song))*0.25
...
def notTooHigh(song):
    fitness = 0
    for note in song:
        if midiNote.getPitch(note) < 75: fitness += 1
